...
class Game extends React.Component {
    ...
    maybeShowWinnerText = () => {
        ...
        if (winnerName === "draw") {
            winnerText = "draw";
            M.toast({ html: "Nice Try But You can't beat the AI" });
        } else {
            winnerText = "You Lose";
        }

        return (
            <div className="game-winner-text page-text center-align">
                {winnerText}
            </div>
        );
    };
    ...
    maybeShowLoader = () => {
        if (
            this.state.myTurn ||
            this.state.gameState !== gameStates.GAME_IN_PROGRESS
        ) {
            return "";
        }
        return preloader();
    ...
    generateGameGrid = () => {
        return generateGameGrid(
            this.state.gameGrid,
            this.state.myTurn,
            this.state.gameState,
            this.state.winCoordinates,
            this.onCellClick
        );
    };
    ...
    render() {
        return (
            <div className="tall-container-grow">
                <div className="game-button-group center-align">
                    <a
                        className="game-button waves-effect waves-light btn white black-text"
                        data-player="0"
                        onClick={this.choosePlayer}
                        helper-text="Play First"
                    >
                        Start First
                    </a>
                    <a
...
ReactDOM.render(<Game />, document.getElementById("game-react-container"));

function parseGameCellId(id) {
    const splitted = id.split("-");
    return [splitted[splitted.length - 2], splitted[splitted.length - 1]].map(
        (value) => {
...
function generateGameRows(
    ...
    return grid.map((row, rowNumber) => {
        return (
            <div className="game-row" key={rowNumber}>
                {generateGameCells(
                    row,
                    rowNumber,
                    isMyTurn,
...
function generateGameCells(
    ...
    return row.map((cellMarker, columnNumber) => {
        ...
        const cellPartOfWinCoordinates = arrayContainsArray(
        ...
        return (
            <div
                key={columnNumber}
                id={generateGameCellId(columnNumber, rowNumber)}
                className={cellStyle(isCellOccupied, cellPartOfWinCoordinates)}
